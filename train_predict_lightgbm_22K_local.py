...
LOCAL VERSION - Optimized for MacBook Air M2 (~20 min runtime)
...
class LightGBMMultiLabelClassifier22KLocal:
    """
    ...
    Optimized for MacBook Air M2 (8 cores, 8-16GB RAM).
    """
    ...
    def get_top_labels(self, labels_list, top_k=500):
        """
    ...
    def build_model(self):
        ...
        Optimized for MacBook Air M2 (8 cores, limited RAM).
        
        Returns:
            MultiOutputClassifier: Configured model
        """
        logger.info("Building LightGBM multi-output model (optimized for M2 local)...")
        
        # LightGBM base classifier optimized for MacBook Air M2
        lgb_classifier = lgb.LGBMClassifier(
            n_estimators=80,           # Reduced for speed
            max_depth=6,               # Shallower trees
            learning_rate=0.1,         # Higher LR with fewer trees
            num_leaves=31,             # Fewer leaves for speed
            ...
            reg_alpha=0.2,
            reg_lambda=0.2,
            force_col_wise=True,       # Better memory footprint
            device_type="cpu",
            random_state=self.random_state,
            n_jobs=4,                  # Limit threads for M2 RAM
            verbose=-1
        )
        
        # Wrap in MultiOutputClassifier for multi-label prediction
        self.model = MultiOutputClassifier(lgb_classifier, n_jobs=4)
    ...
    def save_model(self, output_dir='./lightgbm_22K_local'):
        """
        ...
        model_path = os.path.join(output_dir, 'lightgbm_multilabel_model_22K_local.joblib')
        encoder_path = os.path.join(output_dir, 'label_encoder_22K_local.joblib')
        feature_encoders_path = os.path.join(output_dir, 'feature_encoders_22K_local.joblib')
    ...
    def load_model(self, output_dir='./lightgbm_22K_local'):
        """
...
def main():
    ...
    """
    # Configuration - LOCAL VERSION (faster)
    FULL_DATA_PATH = './data/df_ml2.csv'
    TEST_DATA_PATH = './data/test_df_prediag_auto.csv'
    OUTPUT_DIR = './lightgbm_22K_local'
    RANDOM_STATE = 42
    TOP_K_LABELS = 500  # Reduced from 4001 for ~20 min runtime
    
    # Initialize classifier
    classifier = LightGBMMultiLabelClassifier22KLocal(random_state=RANDOM_STATE)
    ...
    split_path = os.path.join(OUTPUT_DIR, 'test_predictions_22K_local.joblib')
    ...
    card_metrics_path = os.path.join(OUTPUT_DIR, 'cardinality_accuracy_22K_local.csv')
    ...
    compare_path = os.path.join(OUTPUT_DIR, 'pred_vs_true_22K_local.csv')
